...
static LPCTSTR const kPasswordHistory = TEXT("PasswordHistory");
...
static const unsigned kPasswordHistorySize = 16;

static void AddUniquePassword(UStringVector &list, const UString &s)
{
  if (s.IsEmpty())
    return;
  FOR_VECTOR (i, list)
...
void Save_PasswordHistory(const UString &password)
{
  ...
  AddUniquePassword(passwords, password);

  if (passwords.Size() > kPasswordHistorySize)
    passwords.Delete(0);
